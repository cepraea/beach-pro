...
def verify_repository_identity():
    """
    Verifica se o script está sendo executado no repositório correto ('cepraea/beach-pro').
    Lança SystemExit em caso de erro ou repositório incorreto.
    """
    try:
        toplevel = subprocess.check_output(['git', 'rev-parse', '--show-toplevel'], text=True, stderr=subprocess.PIPE).strip()
        remote_url = subprocess.check_output(['git', 'remote', 'get-url', 'origin'], text=True, cwd=toplevel, stderr=subprocess.PIPE).strip()

        # Normaliza a URL para o formato owner/repo
        match = re.search(r'github\.com[/:]([^/]+/[^/]+?)(\.git)?$', remote_url)
        if not match:
            raise SystemExit(f"FAIL: Não foi possível normalizar a URL do remote: {remote_url}")

        repo_id = match.group(1)
        expected_id = "cepraea/beach-pro"

        if repo_id != expected_id:
            raise SystemExit(f"FAIL: WRONG_REPOSITORY. Esperado '{expected_id}', observado '{repo_id}'.")
    except subprocess.CalledProcessError as e:
        raise SystemExit(f"FAIL: Erro ao executar comando git para verificar o repositório: {e.stderr.strip()}")
