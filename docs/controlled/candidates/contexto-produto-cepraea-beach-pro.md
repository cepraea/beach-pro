...
permitted_uses:
  - revisao_da_versao_0_2
  - analise_de_impacto
prohibited_uses:
  - uso_como_documento_vigente
...
# CONTEXTO CEPRAEA BEACH PRO — VERSÃO DE TRABALHO 0.2

- [CONTEXTO CEPRAEA BEACH PRO — VERSÃO DE TRABALHO 0.2](#contexto-cepraea-beach-pro--versão-de-trabalho-02)
  ...
  - [11. Pendências, contradições e decisões](#11-pendências-contradições-e-decisões)
    - [11.1 Decisões de produto](#111-decisões-de-produto)
      ...
      - [`DEC-017`](#dec-017)
      - [`DEC-018`](#dec-018)
    - [11.2 Limitações do sistema legado que fundamentam a PWA](#112-limitações-do-sistema-legado-que-fundamentam-a-pwa)
...
- VERSÃO DE TRABALHO: `0.2` — NÃO VIGENTE. Derivada integralmente da versão
  `0.1`, hash
  `71bd2695280f0cdd5c41b83c7e433d5a84a803b527a7e09d7dfd7eecaaeab847`.
  Esta versão acumula correções encontradas durante a revisão dos requisitos.
  Até nova aprovação explícita, a versão `0.1` permanece vigente.
...
### 11.2 Limitações do sistema legado que fundamentam a PWA

Os itens `CORR-001` a `CORR-007` foram observados nas planilhas e interfaces
legadas durante a descoberta do funcionamento atual. Eles não constituem
backlog obrigatório de manutenção, correção ou evolução das planilhas. Sua
função documental é demonstrar limitações do `AS-IS`, justificar decisões da
PWA e impedir que o produto novo repita os mesmos problemas.

- `CORR-001` — A divergência entre o retorno confirmado de Gilvania e as
  representações digitais evidencia ausência de fonte operacional única para o
  elenco. Consequência para a PWA: cadastro estruturado governado pela decisão
  humana de Davi e reconciliação controlada durante eventual migração.
- `CORR-002` — A mistura entre disponibilidade declarada e presença real
  evidencia falha semântica do modelo legado. Consequência para a PWA: registros
  distintos conforme `RF-012`, `RF-013` e `RF-014`.
- `CORR-003` — Estados e vocabulários divergentes evidenciam falta de linguagem
  operacional comum. Consequência para a PWA: vocabulário controlado, estados
  inequívocos e regras temporais consistentes.
- `CORR-004` — A presença de comissão, coordenação e outros atores inexistentes
  evidencia desconexão entre a interface e a organização real. Consequência
  para a PWA: somente Davi e atleta como perfis operacionais da primeira fase,
  conforme `DEC-017` e `RF-038`.
- `CORR-005` — Fórmulas quebradas e painéis vencidos evidenciam fragilidade das
  derivações e risco de apresentar informação desatualizada. Consequência para
  a PWA: estado temporal verificável, fatos validados e indicadores que não
  funcionam como fonte primária.
- `CORR-006` — Identificadores duplicados e pendências de logs evidenciam baixa
  rastreabilidade. Consequência para a PWA: identificadores únicos, autoria,
  histórico e auditoria conforme `RF-033` a `RF-036`.
- `CORR-007` — Edição por link e comentários históricos com conteúdo pessoal
  evidenciam risco de integridade e privacidade. Consequência para a PWA: contas
  individuais, RLS, acesso mínimo e visibilidade restrita conforme `RF-037`,
  `RF-045` e `RF-046`.

Enquanto as planilhas permanecerem em uso, uma correção pontual somente é
necessária como contenção operacional quando houver risco imediato para a
operação, a integridade ou a privacidade. Essa contenção não transforma as
planilhas no frontend a ser evoluído nem altera a decisão de substituí-las pela
PWA.
